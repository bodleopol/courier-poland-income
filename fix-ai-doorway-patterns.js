...
// Natural Ukrainian alternatives for common repeated phrases
const alternatives = {
  ...
  "Додаткові вихідні за понаднормову роботу": [
    ...
    "За понаднормові годині або гроші, або вихідний — вибираєш сам",
...
async function main() {
  try {
    ...
    console.log(`Found ${content.length} vacancies.`);
    console.log('Replacing repeated AI doorway phrases...');
    
    let replacementCount = 0;
    const enhanced = content.map((vacancy, index) => {
      const originalOffers = JSON.stringify(vacancy.offers_ua);
      const updated = replaceRepeatedPhrases(vacancy);
      const newOffers = JSON.stringify(updated.offers_ua);
      
      if (originalOffers !== newOffers) {
        replacementCount++;
      }
    ...
    console.log('✅ AI doorway patterns fixed!');
    console.log(`   - Processed ${enhanced.length} vacancies`);
    console.log(`   - Updated ${replacementCount} vacancies`);
  } catch (error) {
