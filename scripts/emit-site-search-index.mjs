...
function readCatalogHtml(filePath) {
...
function load(lang, specialistsFile, startupsFile) {
  ...
  const specHtml = readCatalogHtml(specPath);
  const startHtml = readCatalogHtml(startPath);
...
const body = `/* Built from src/pages/site via scripts/emit-site-search-index.mjs (node build.js) */\nwindow.SITE_SEARCH_INDEX=${json};\n`;
