...
const extraSpecialistCountries = [
  t('Україна', 'Ukraine', 'Украина', 'Ucrania'),
  t('Іспанія', 'Spain', 'Испания', 'España'),
  t('Німеччина', 'Germany', 'Германия', 'Alemania'),
  t('Нідерланди', 'Netherlands', 'Нидерланды', 'Países Bajos'),
  t('Канада', 'Canada', 'Канада', 'Canadá'),
  t('Велика Британія', 'United Kingdom', 'Великобритания', 'Reino Unido')
];

const extraSpecialists = Array.from({ length: 100 }, (_, index) => {
  const n = index + 1;
  const level = (n % 5) + 1;
  const country = extraSpecialistCountries[index % extraSpecialistCountries.length];
  return {
