...
(function() {
  ...
  const translations = {
    ...
    'bogdan.subtitle': { ua: 'Інвестор, Директор, Голова та радник', pl: 'Inwestor, Dyrektor, Prezes i Doradca', ru: 'Инвестор, Директор, Председатель и советник', en: 'Investor, Director, Chairman and Advisor' },
    'bogdan.intro': { ua: 'Я українець, проживаю у прекрасному місті Львів. Наразі я інвестую у нові платформи, а також працюю операційним директором в міжнародній логістичній компанії. Мій шлях у бізнесі та інвестиціях навчив мене головному: найцінніший актив будь-якого проекту — це люди. Саме тому я приєднався до команди Rybezh як Інвестор та Директор.', pl: 'Jestem Ukraińcem, mieszkam w pięknym mieście Lwów. Obecnie inwestuję w nowe platformy i pracuję jako dyrektor operacyjny w międzynarodowej firmie logistycznej. Moja droga w biznesie i inwestycjach nauczyła mnie jednego: najcenniejszym aktywem każdego projektu są ludzie. Dlatego dołączyłem do zespołu Rybezh jako Inwestor i Dyrektor.', ru: 'Я украинец, проживаю в прекрасном городе Львов. В настоящее время я инвестирую в новые платформы, а также работаю операционным директором в международной логистической компании. Мой путь в бизнесе и инвестициях научил меня главному: самый ценный актив любого проекта — это люди. Именно поэтому я присоединился к команде Rybezh как Инвестор и Директор.', en: 'I am Ukrainian, living in the beautiful city of Lviv. Currently, I invest in new platforms and also work as an operational director in an international logistics company. My journey in business and investments taught me the main thing: the most valuable asset of any project is its people. That is why I joined the Rybezh team as an Investor and Director.' },
    'bogdan.mission': { ua: 'Наша платформа створена для того, щоб зробити пошук роботи в Польщі прозорим, безпечним та максимально комфортним для кожного українця. Ми постійно вдосконалюємось, впроваджуємо нові технології та розширюємо можливості. Як Голова та радник, я відповідаю за стратегічний розвиток та нові партнерства.', pl: 'Nasza platforma została stworzona, aby wyszukiwanie pracy w Polsce było przejrzyste, bezpieczne i maksymalnie komfortowe dla każdego Ukraińca. Stale się doskonalimy, wprowadzamy nowe technologie i poszerzamy możliwości. Jako Prezes i Doradca odpowiadam za rozwój strategiczny i nowe partnerstwa.', ru: 'Наша платформа создана для того, чтобы сделать поиск работы в Польше прозрачным, безопасным и максимально комфортным для каждого украинца. Мы постоянно совершенствуемся, внедряем новые технологии и расширяем возможности. Как Председатель и советник, я отвечаю за стратегическое развитие и новые партнерства.', en: 'Our platform is created to make job searching in Poland transparent, safe, and as comfortable as possible for every Ukrainian. We are constantly improving, implementing new technologies, and expanding opportunities. As Chairman and Advisor, I am responsible for strategic development and new partnerships.' },
    ...
    'bogdan.sidebar_role': { ua: 'Інвестор та Операційний Директор', pl: 'Inwestor i Dyrektor Operacyjny', ru: 'Инвестор и Операционный Директор', en: 'Investor and Operational Director' },
  ...
  function initVirtualAssistant() {
    ...
    async function loadVaData() {
      if (vaData || isFetching) return;
      isFetching = true;
      try {
        const response = await fetch('/va-data.json');
        if (response.ok) {
          vaData = await response.json();
        } else {
          console.error('Failed to load VA data');
        }
      } catch (err) {
    ...
    fab.addEventListener('click', async () => {
      ...
      if (!hasOpened) {
        ...
        // Ensure data is loaded to get initial greet from parameters if possible
        await loadVaData();

        let initialGreet = 'Привіт! Чим можу допомогти?';
        if (vaData && vaData.length > 0) {
           initialGreet = vaData[0][`r_${lang}`] || vaData[0]['r_ua'] || initialGreet;
        }
    ...
    function getBotReply(userMsg) {
      ...
      const defaultReply = fallbackObj[lang] || fallbackObj['ua'] || 'Вибачте, я не зрозумів.';

      if (!vaData) return defaultReply;

      const msgLower = userMsg.toLowerCase();

      for (const param of vaData) {
