...
const RU_FALLBACK_REPLACEMENTS = [
  ...
  ['IT та технології', 'IT и технологии'],
...
function toRussianFallbackText(input) {
  ...
  const wordReplacements = [
    ...
    [/\bпрацівник\b/gi, 'сотрудник'],
    [/\bпрацивникив\b/gi, 'сотрудников'],
    [/\bпрацівників\b/gi, 'сотрудников'],
    [/\bстабильний\b/gi, 'стабильный'],
    ...
    [/\bвакансія за відгуками\b/gi, 'вакансия по отзывам'],
    [/\bвси\b/gi, 'все'],
    [/\bвсі\b/gi, 'все'],
    [/\bкракив\b/gi, 'Краков'],
    [/\bкраків\b/gi, 'Краков'],
    ...
    [/\bдопомагаємо\b/gi, 'помогаем'],
    [/\bдопоможемо\b/gi, 'поможем'],
    [/\bперевирка\b/gi, 'проверка'],
    [/\bперевірка\b/gi, 'проверка'],
    [/\bконтроль якости\b/gi, 'контроль качества'],
    [/\bконтроль якості\b/gi, 'контроль качества'],
    [/\bвидпусток\b/gi, 'отпусков'],
    [/\bвідпусток\b/gi, 'отпусков'],
    [/\bликарняних\b/gi, 'больничных'],
    [/\bлікарняних\b/gi, 'больничных'],
    [/\bПошук\b/gi, 'Поиск'],
    ...
    [/\bвідгукнутися\b/gi, 'откликнуться'],
    [/\bвидгук\b/gi, 'отзыв'],
    [/\bвідгук\b/gi, 'отзыв'],
    [/\bдосвид\b/gi, 'опыт'],
    [/\bдосвід\b/gi, 'опыт'],
...
async function build() {
  ...
  // copy static pages
  const staticPages = [
  'respond.html','apply.html', 'about.html', 'contact.html', 'privacy.html', 'terms.html', 'company.html', 'faq.html', '404.html', 'calculator.html', 'cv-generator.html', 'red-flag.html', 'map.html', 'proof.html', 'for-employers.html', 'press.html', 'rent.html', 'quiz.html', 'tinder.html', 'tinder-ru.html', 'apply-ru.html', 'about-ru.html', 'contact-ru.html', 'privacy-ru.html', 'terms-ru.html', 'company-ru.html', 'faq-ru.html', 'calculator-ru.html', 'cv-generator-ru.html', 'red-flag-ru.html', 'map-ru.html', 'proof-ru.html', 'for-employers-ru.html', 'blog-ru.html', 'vacancies-ru.html', 'index-ru.html', 'game.html', 'quiz-ru.html', 'bogdan-tiutenko.html'];
  for (const p of staticPages) {
    try {
      ...
      if (!pContent.includes('<html')) {
        ...
        } else if (p.startsWith('tinder')) {
          title = 'Tinder Вакансій | Свайпай та зберігай роботу в Польщі';
          desc = 'Інноваційний пошук роботи в стилі Tinder. Свайпай вправо, щоб зберегти вакансію у кошик, та подай заявку на всі одразу.';
        }
  ...
  for (let page = 1; page <= totalPages; page++) {
    ...
    blogHtml = blogHtml.replace(
      ...
      '<meta name="keywords" data-i18n="blog.meta_keywords" data-i18n-attr="content" content="'
    );
    blogHtml = blogHtml.replace(
      '<meta property="og:title" content="',
  ...
  for (const post of posts) {
    ...
    let postHtml = pageTpl
      ...
      .replace(/{{KEYWORDS}}/g, escapeHtml(post.keywords || 'робота в польщі, вакансії польща, працевлаштування'))
...
// ALL_JOBS loaded dynamically from /jobs-data.json via jobs-loader.js
    ...
    indexHtml = indexHtml.replace('<meta name="keywords" content="', '<meta name="keywords" data-i18n="meta.keywords" data-i18n-attr="content" content="');
...
// ALL_JOBS loaded dynamically from /jobs-data.json via jobs-loader.js
      ...
      vacanciesHtml = vacanciesHtml.replace(
        '<meta name="keywords" content="',
        '<meta name="keywords" data-i18n="vacancies.meta_keywords" data-i18n-attr="content" content="'
      );
      vacanciesHtml = vacanciesHtml.replace(
        '<meta property="og:title" content="',
  ...
  // Copy tinder assets
  try {
    const tinderJsContent = await fs.readFile(path.join(SRC, 'tinder.js'), 'utf8');
    await fs.writeFile(path.join(DIST, 'tinder.js'), tinderJsContent, 'utf8');
  } catch (e) {
    console.error('Error copying tinder.js:', e);
  }
